...
- 「帳票監査モード設定」で数量読取を有効にした場合、薬品名と同じ行の右側にある数量を優先して対応付ける。横向きでOCR座標の並びが変わった場合は、薬品名の上下で横位置が重なる数量も対象にする。
- OCRが数字と単位を別行へ分けた場合は、近接する数字と数量単位を再結合して数量とする。薬品名と数量は互いに一意な最短候補の場合だけ対応付け、曖昧な数量を別薬品へずらして表示しない。
- 横向きの帳票で数量が薬品名の下側に配置される場合は、OCR座標を右から左へ並べて、帳票の上から下の順番を復元する。数量が上側の場合は左から右へ並べる。
...
- 帳票監査のPTP読取カメラは、調剤モードのPTP読取と同じ `2.0倍` の初期倍率を使用する。
