...
1. `BarcodeAnalyzer` が zxing-cpp の `sequenceIndex / sequenceSize / sequenceId` と `bytes` を保持しているか。
2. 同じ Structured Append グループの順序番号が 0 から総数-1 まで揃っているか。
3. `JahisQrAssembler` が各断片を先に文字列化せず、生バイト列を順序どおりに連結してから Windows-31J デコードしているか。
4. Structured Append がない単独QR・No.911形式で ML Kit と位置順フォールバックが動作しているか。
5. `checkCompleteness()` が終端改行を要求していないか。
6. `JahisQrParser` が `JAHISTC07` をTC01系として扱っているか。
7. `201` と `301` の RP番号が揃っているか。
